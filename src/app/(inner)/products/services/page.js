        ...
        async function load() {
            try {
                ...
                if (!res.ok || !json || json.ok !== true) {
                    const message = json?.error || "Failed to load service products";
                    throw new Error(message);
                }
                if (!cancelled) setProducts(Array.isArray(json.data) ? json.data : []);
            } catch (e) {
                if (!cancelled) setError(e instanceof Error ? e.message : "Failed to load service products");
            } finally {
                        ...
                        <div className="row">
                            <div className="col-lg-12">
                                <div className="career-page-single-banner blog-page">
                                    <h1 className="title">Service Products</h1>
                                </div>
                            </div>
                        </div>
                    </div>
