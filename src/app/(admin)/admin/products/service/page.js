...
export default function Page() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const res = await fetch("/api/admin/products/service", { cache: "no-store" });
      if (!res.ok) {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const url = isEditing ? `/api/admin/products/service/${editingId}` : "/api/admin/products/service";
  ...
  async function onDelete(item) {
    setSuccess("");
    setError("");
    const ok = window.confirm(`Delete "${item?.name || "this service"}"?`);
    if (!ok) return;

    try {
      const res = await fetch(`/api/admin/products/service/${item.id}`, { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body p-0">
          <div className="p-4 p-lg-5 pb-3 d-flex align-items-start justify-content-between gap-3 flex-wrap">
            <div>
              <div className="h6 mb-1">Services</div>
              <div className="text-muted small">All services list and management.</div>
            </div>
            {isEditing ? (
          ...
          <div className="px-4 px-lg-5 pb-4">
            ...
            <div className="formCard mt-3">
              <div className="d-flex align-items-start justify-content-between gap-2 flex-wrap mb-3">
                <div>
                  <div className="h5 mb-1">Service</div>
                  <div className="text-muted small">Add and manage services.</div>
                </div>
              </div>
              ...
              <form onSubmit={onSubmit}>
                <div className="row g-3">
                  ...
                  <div className="col-12">
                    <label className="form-label fw-semibold">Service Short Description</label>
                    <input
                      className="form-control"
                  ...
                  <div className="col-12">
                    <label className="form-label fw-semibold">Service Long Description</label>
                    <textarea
                      className="form-control longDesc"
                      rows={6}
                      value={form.longDesc}
                      onChange={(e) => setForm((f) => ({ ...f, longDesc: e.target.value }))}
                      placeholder="Write a detailed, professional description of the service..."
                      disabled={isSaving}
                    />
                  </div>

                  <div className="col-12 col-lg-6">
                    <label className="form-label fw-semibold">Service Thumbnail</label>
                  ...
                  <div className="col-12 col-lg-6">
                    <label className="form-label fw-semibold">Service Images (Multiple)</label>
                    <input
                      className="form-control"
                      type="file"
                      accept="image/*"
                  ...
                  <div className="col-12 col-md-4">
                    <label className="form-label fw-semibold">Service Price (Starting Price)</label>
                    <input
                      className="form-control"
                      type="number"
                      step="0.01"
          ...
          <div className="table-responsive">
            <table className="table align-middle mb-0">
              <thead className="table-light">
                <tr>
                  <th style={{ width: 90 }}>Service Image</th>
                  <th>Service Name</th>
                  <th style={{ width: 160 }}>Starting Price</th>
